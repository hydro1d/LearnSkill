...
import { User, Mail, Lock, Image, UserPlus, GraduationCap } from "lucide-react";
...
function RegisterForm() {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      const { data, error } = await authClient.signUp.email({ name, email, password, image: photoUrl || undefined });
    ...
    <div className="w-full max-w-md">
      <div className="bg-white border border-stone-100 rounded-3xl shadow-sm p-8">
        ...
        <div className="text-center mb-7">
          ...
          <p className="text-stone-500 text-xs mt-1 font-medium">Start your elite-grade learning journey today.</p>
        </div>


        <form onSubmit={handleRegister} className="space-y-4">
          {[
            { label: "Full Name", type: "text", placeholder: "Alex Mercer", value: name, onChange: setName, Icon: User, required: true },
            { label: "Email Address", type: "email", placeholder: "alex@example.com", value: email, onChange: setEmail, Icon: Mail, required: true },
            { label: "Photo URL (Optional)", type: "url", placeholder: "https://example.com/photo.jpg", value: photoUrl, onChange: setPhotoUrl, Icon: Image, required: false },
            { label: "Password", type: "password", placeholder: "••••••••", value: password, onChange: setPassword, Icon: Lock, required: true },
          ].map(({ label, type, placeholder, value, onChange, Icon, required }) => (
            <div key={label} className="form-control">
              <label className="label py-1"><span className="label-text text-stone-700 text-xs font-bold">{label}</span></label>
              <div className="relative">
                ...
                <input type={type} placeholder={placeholder} value={value}
                  ...
                  className="input input-bordered w-full pl-10 bg-stone-50 border-stone-200 text-stone-900 text-sm rounded-2xl focus:border-orange-500 focus:outline-none transition-all placeholder:text-stone-400" />
              </div>
            </div>
          ))}
        ...
        <button onClick={handleGoogleLogin} disabled={loading || socialLoading}
          ...
          {socialLoading
            ? <span className="w-5 h-5 rounded-full border-2 border-orange-500 border-t-transparent animate-spin"></span>
            : <>
        ...
        <div className="text-center mt-6 text-stone-500 text-xs font-medium">
          ...
          <Link href="/login" className="text-orange-600 hover:text-orange-700 font-bold transition-colors">Login here</Link>
        </div>
      </div>
    </div>
  );
